...
class TotalClassView(APIView):
    # permission_classes = (IsAdminUser, IsAuthenticated)
    def get(self, request):
        return Response(Classroom.objects.count())
...
class ClassroomView(APIView, PaginationHandlerMixin):
    ...
    @swagger_auto_schema(
        manual_parameters=[
        openapi.Parameter('id', openapi.IN_QUERY, type=openapi.TYPE_INTEGER, description='Classroom id'),
        openapi.Parameter('sort', openapi.IN_QUERY, type=openapi.TYPE_STRING, description='class_name, location')],
    )
    def get(self, request):
        ...
        if id:
            classrooms = classrooms.filter(pk=id)
        if sort:
...
class TimetableView(APIView, PaginationHandlerMixin):
    ...
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter('id', openapi.IN_QUERY, type=openapi.TYPE_INTEGER, description='Timetable id'),
            openapi.Parameter('sort', openapi.IN_QUERY, type=openapi.TYPE_STRING, description='day_of_week, shifts, semester, school_year')],
    )
    def get(self, request):
        ...
        if id:
            timetables = timetables.filter(pk=id)
        if sort:
...
class ClassRecordView(APIView, PaginationHandlerMixin):
    ...
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter('id', openapi.IN_QUERY, description="Class record id", type=openapi.TYPE_INTEGER),
